...
const ChatBox = () => {
    ...
    const {recipientUsers} = useFetchRecipientUsers(currentChat, user);
    const [textMessage, setTextMessage] = useState("");

    const isGroupChat = currentChat?.name;
    ...
    return (<>
        ...
        <div className="chat-header">
            {!chatDeleted ? 
            (isGroupChat ? <strong>{currentChat.name}</strong> : <strong>{recipientUser.name}</strong>)
             : (<strong>Other user logged out, chat was deleted :&#40;</strong>)}
        </div>  
        <div className="conversation">
            {messages && messages.map((message,index) => (
                <div className="message" key ={index}>
                {isGroupChat && (message?.senderId !== user?._id) && <span className="sender">{message?.senderName}</span>}
